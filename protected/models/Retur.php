...
class Retur extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('no_po,nama_barang, dipesan, jumlah, tujuan, keterangan, status, tanggal, user_id', 'required'),
			array('jumlah, dipesan, user_id', 'numerical', 'integerOnly'=>true),
			array('nama_barang', 'length', 'max'=>40),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, no_po, nama_barang, dipesan, jumlah, tujuan, keterangan, status, tanggal, user_id', 'safe', 'on'=>'search'),
		);
	}
